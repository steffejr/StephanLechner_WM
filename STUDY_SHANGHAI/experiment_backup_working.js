// Experiment configuration - SIMPLE like finger tapping
const jsPsych = initJsPsych({
    on_finish: function() {
        jatos.endStudy(); // Just end, saveData already called
    },
...
// saveData function - EXACT copy of finger tapping pattern
function saveData() {
...
const experimentalTrials = {
    ...
    repetitions: 3, // For testing
    randomize_order: false
};

// Save data trial - WAIT for data to be sent before ending
const save_data_trial = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus: '<div style="color: white;">Saving data...</div>',
    choices: "NO_KEYS",
    trial_duration: null, // Don't auto-advance
    on_start: function() {
        // Call saveData which will end the trial when done
        saveDataAndFinish();
    }
};

// NEW function that properly waits for JATOS
function saveDataAndFinish() {
    console.log("=== saveDataAndFinish() CALLED ===");
    ...
    // JATOS submission
    if (typeof jatos !== 'undefined') {
        console.log("Submitting to JATOS...");
        
        // Submit CSV data - THIS is what creates data.txt
        jatos.submitResultData(csvContent, "text/csv");
        
        // Also submit metadata
        const metadata = {
            subject_id: sub_info.subID,
            date: new Date().toISOString(),
            total_trials: trialNumber - 1
        ...
        console.log("Data submitted, finishing trial in 1 second...");
        
        // Wait for data to be sent, then finish
        setTimeout(function() {
            jsPsych.finishTrial();
        }, 1000);
    } else {
        console.log("JATOS not detected - browser mode");
        setTimeout(function() {
            jsPsych.finishTrial();
        }, 500);
    }
...
// End screen - call saveData HERE like finger tapping does
const end_screen = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus: `
    ...
    trial_duration: 2000,
    on_start: function() {
        // Call saveData when this screen appears
        console.log("End screen started, calling saveData");
        saveData();
    }
};

// Build timeline - REMOVE the save_data_trial, we save in experimentalTrials now
timeline.push(instructions);
timeline.push(experimentalTrials);
// timeline.push(save_data_trial); // REMOVED
